...
static int sprd_pcm_open(struct snd_pcm_substream *substream)
{
	...
	wake_lock_timeout(&pcm_open_wakelock, msecs_to_jiffies(600));
...
{
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		sp_asoc_pr_info("%s playback\n", __func__);
	else
		sp_asoc_pr_info("%s capture\n", __func__);
	return 0;
}
...
{
	wake_lock_init(&pcm_open_wakelock,
				WAKE_LOCK_SUSPEND, "pcm_open_wakelock");
