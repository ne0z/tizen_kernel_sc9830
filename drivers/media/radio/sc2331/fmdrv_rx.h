...
typedef struct _FM_SEARCH_PAR_T
		{
		unsigned short startFreq;
		unsigned short endFreq;
		unsigned char  rssiThresh;
		unsigned char direction;
		unsigned char scanMode;
		unsigned char multiChannel;
		unsigned char condType;
		unsigned char condValue;
}FM_SEARCH_PAR_T;

//unsigned short global_frequency;
...
int fm_rx_set_Frequency_Offset_threshold(struct fmdrv_ops*, short);
int fm_rx_set_Noise_Power_threshold(struct fmdrv_ops*, short);
int fm_rx_set_Pilot_Power_threshold(struct fmdrv_ops*, short);
...
int fm_rx_enable_rds(struct fmdrv_ops *, u8);
...
int fm_rx_cfg_blend_softmute (struct fmdrv_ops*,struct fm_blend_soft_mute*);
#endif
