...
int sensor_k_open(struct inode *node, struct file *file)
{
	...
	/* check current user */
	printk("current camera users: %d\n", atomic_read(&p_mod->total_users));
...
LOCAL long sensor_k_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SENSOR_IO_IF_CFG:
		...
		case SENSOR_IO_POWER_CFG:
		{
			...
			if (0 == ret) {
#if defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_J1ACEVELTE)
				if(1 == pwr_cfg.is_on) {
...
#if defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) 		// Set Front Camera GPIO value for DVS
...
{
#if defined(CONFIG_MACH_J3XLTE) || defined(CONFIG_MACH_J3XNLTE) || defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE)
...
{
#if defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE)
