/* drivers/power/load_analyzer_sc7727se.c */
...
void store_cpu_load(unsigned int cpufreq[], unsigned int cpu_load[])
{
	...
	cpu_load_freq_history[cnt].gpu_freq
					= saved_load_factor.gpu_freq / 1000;
...
char cpu_load_freq_menu[] ="=======================================" \
			...
			"CPU0   CPU1   CPU2   CPU3   ONLINE   NR_RUN\n";
...
unsigned int show_cpu_load_freq_sub(int cnt, int show_cnt, char *buf, unsigned int buf_size, int ret)
{
	...
	for (j = 0; j < show_cnt; j++) {
		...
		ret +=  snprintf(buf + ret, PAGE_SIZE - ret
			, "%10s\t%d.%d    %d.%d/%d.%d   %d/%d    [%5d]    %3d    %3d    %3d    %3d    "
				"%3d    %3d.%02d\n"
...
char cpu_bus_load_freq_menu[] ="==============================" \
			...
			"CPU0 ( C1/ C2/ C3)    CPU1    CPU2    CPU3   ONLINE    NR_RUN    MIF_F    MIF_L" \
...
char cpu_bus_clk_load_freq_menu[] ="============================" \
			...
			"============================================\n" \
			"   TIME    STA  CPU_F   F_LOCK   O_LOCK  [INDEX]    " \
			"CPU0 ( C1/ C2/ C3)    CPU1   ONLINE    NR_RUN    MIF_F    MIF_L" \
			"    GPU_F  GPU_UTIL   INPUT" \
...
#if defined (CONFIG_SLP_BUSY_LEVEL)
static int gpu_very_busy_usage = 240;
static int gpu_busy_usage = 150;

int gpu_load_checking(unsigned int cnt)
{
	int ret = NOT_BUSY_LOAD;

	if (cpu_load_freq_history[cnt].gpu_utilization >= gpu_very_busy_usage) {
		ret = GPU_VERY_BUSY_LOAD;
	} else if (cpu_load_freq_history[cnt].gpu_utilization >= gpu_busy_usage) {
		ret = BUSY_LOAD;
	}
...
int cpu_load_checking(unsigned int cnt)
{
	...
	if ((high_cpufreq >= 1500000) && (high_usage >= 90))
		ret = BUSY_LOAD;
