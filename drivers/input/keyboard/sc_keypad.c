...
static DEVICE_ATTR(reset_enable, 0664,reset_enable_show, reset_enable_store);


static ssize_t reset_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int reg_val, ret_val = 0;
...
{				//TODO: if usign gpio(eic), need add row , cols to platform data.
	static unsigned long last_value = 0xff;
	...
	if (last_value == value) {
		...
		input_sync(sci_kpd->input_dev);
		printk("%s: seems an event is missing, just report it\n", __func__);
		sci_keypad_keydbg(key, last_value);
	}
	...
	input_report_key_hook(sci_kpd->input_dev, key, value);
#endif
	input_report_key(sci_kpd->input_dev, key, value);
	input_sync(sci_kpd->input_dev);
...
static int sci_keypad_probe(struct platform_device *pdev)
{
	...
	powerkey_dev = device_create(sec_class, NULL, (dev_t)&powerkey_dev, NULL, "sec_powerkey");
	if (device_create_file(powerkey_dev, &dev_attr_sec_powerkey_pressed) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_sec_powerkey_pressed.attr.name);

	if(device_create_file(powerkey_dev, &dev_attr_reset_enable) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_reset_enable.attr.name);
