...
enum EDGE_CORRECTION_TYPE {
	EDGE_NORMAL = 0,
	EDGE_NO_CORR,
	EDGE_MAX_CORR,
};

const char* cmd_status_str[CMD_STATUS_MAX] = {
	"WAITING",
	"RUNNING",
	"OK",
	"FAIL",
...
static void cmd_get_fw_ver_bin(void *device_data)
{
	...
	sprintf(buf, "ME%02X%04X", fw->data[fw->size - 31], *(u16 *)&fw->data[fw->size - 42]);
	info->cmd_state = CMD_STATUS_OK;
...
static void cmd_get_fw_ver_ic(void *device_data)
{
	...
	sprintf(offset, "%02X%02X",  param_ver[0], param_ver[1]);
	info->cmd_state = CMD_STATUS_OK;
...
static void cmd_get_cm_delta(void *device_data)
{
	...
	char buf[512] = {0, };
	char tmp[16] = {0, };
	const int key_node = info->node_y;
	const int dummy_node = key_node +1;
	int node = info->cmd_param[0];
	int i;

	cmd_clear_result(info);

	if((node < 0) || (node > dummy_node)){
		dev_err(&info->client->dev, "%s: invalid node value [%d]\n", __func__, node);
		sprintf(buf, "%s", "NG");
		info->cmd_state = CMD_STATUS_FAIL;
		goto EXIT;
	}
	...
	if (node == key_node) {
		for (i=0; i < USE_KEY_NUM; i++) {
			sprintf(tmp, "%d",  info->image_buf[i + (info->node_x * info->node_y)]);
			strcat(buf, tmp);
			if (i < USE_KEY_NUM - 1)
				strcat(buf, " ");
		}
	} else if (node == dummy_node) {
		for (i=0; i < DUMMY_KEY_NUM; i++) {
			sprintf(tmp, "%d",
				info->image_buf[i + USE_KEY_NUM + (info->node_x * info->node_y)]);
			strcat(buf, tmp);
			if (i < DUMMY_KEY_NUM - 1)
				strcat(buf, " ");
		}
	} else {
		for (i=0; i < info->node_x; i++) {
			sprintf(tmp, "%d",  info->image_buf[i + (info->node_x * node)]);
			strcat(buf, tmp);
			if (i < info->node_x - 1)
				strcat(buf, " ");
		}
...
static void cmd_edge_mode(void *device_data)
{
	...
	cmd_clear_result(info);
 	...
 	if((val == 0) || (val == 1)){
		...
		wbuf[2] = val ? EDGE_NORMAL : EDGE_MAX_CORR;
		if(mip_i2c_write(info, wbuf, 3)){
			dev_err(&info->client->dev, "%s [ERROR] mip_i2c_write\n", __func__);
			goto ERROR;
		}
...
/**
* Command : Get connector status (screen)
*/
static void cmd_get_connect_test(void *device_data)
{
	struct mip_ts_info *info = (struct mip_ts_info *)device_data;
	char buf[64] = { 0 };
	u8 rbuf[64];
...
static struct mip_cmd mip_commands[] = {
	...
	{ MIP_CMD("tsp_connect_test", cmd_get_connect_test),   },
...
static ssize_t mip_sys_cmd(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
	...
	dev_info(&info->client->dev, "%s:[%s][%s]\n",
		__func__, info->cmd_result, cmd_status_str[info->cmd_state]);
...
ERROR:
	if (info) {
		cmd_clear_result(info);
		info->cmd_state = CMD_STATUS_FAIL;
		cmd_set_result(info, "NG", strnlen("NG", sizeof("NG")));
	}
...
static ssize_t mip_sys_cmd_status(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	if (info->cmd_state >= CMD_STATUS_MAX)
		info->cmd_state = CMD_STATUS_NONE;

	dev_info(&info->client->dev, "%s:[%s][%s]\n",
		__func__, info->cmd_result, cmd_status_str[info->cmd_state]);

	return snprintf(buf, PAGE_SIZE, "%s\n", cmd_status_str[info->cmd_state]);
}
...
static ssize_t mip_sys_store_debug(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
	struct mip_ts_info *info = dev_get_drvdata(dev);
	int ret;

	ret = strtobool(buf, &info->debug);
	if (ret) {
		dev_err(&info->client->dev, "%s: Failed to get enable value.\n", __func__);
		return ret;
	}
...
static ssize_t mip_sys_show_debug(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct mip_ts_info *info = dev_get_drvdata(dev);
...
static DEVICE_ATTR(debug, 0666, mip_sys_show_debug, mip_sys_store_debug);
...
static ssize_t mip_sys_threshold_key(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	dev_info(&info->client->dev, "%s - threshold [%d]\n", __func__, threshold);
