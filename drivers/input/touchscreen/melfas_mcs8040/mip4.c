...
EXIT:
	dev_info(&info->client->dev, SEC_TOUCHSCREEN" - Enabled\n");
...
EXIT:
	dev_info(&info->client->dev, SEC_TOUCHSCREEN" - Disabled\n");
...
int mip_init_config(struct mip_ts_info *info)
{
	...
	sprintf(info->fw_ver_str, "%02X.%02X %02X.%02X %02X.%02X %02X.%02X", info->fw_version[0], info->fw_version[1], info->fw_version[2], info->fw_version[3], info->fw_version[4], info->fw_version[5], info->fw_version[6], info->fw_version[7]);
	//Resolution
	wbuf[0] = MIP_R0_INFO;
...
static int mip_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = devm_request_threaded_irq(&client->dev, client->irq, NULL, mip_interrupt, IRQF_TRIGGER_FALLING | IRQF_ONESHOT, SEC_TOUCHSCREEN, info);
	if (ret) {
	...
	if (sysfs_create_link(NULL, &client->dev.kobj, SEC_TOUCHSCREEN)) {
...
{
	if (!boot_panel_id) {
		pr_err("%s: Front panel is not detected\n", __func__);
		return -1;
	} else
		pr_info("%s: boot_panel_id: 0x%06x\n", __func__, boot_panel_id);
