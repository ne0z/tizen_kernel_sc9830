		...
		if (type == MIP_EVENT_INPUT_TYPE_KEY) {
...
#if MIP_USE_KEY
				input_report_key(info->input_tkey, info->key_code[id - 1], state);
#endif
				dev_info(&info->client->dev, "%s:[%s] %s\n", __func__, state ? "P":"R", info->key_disc[id - 1]);
			}
			else{
			...
			if(state == 1){
				...
				if (!info->touch_state[id]) {
					...
					if (info->finger_cnt < MAX_FINGER_NUM)
						info->finger_cnt++;
					dev_info(&info->client->dev, "%s: [P][%d] x=%3d, y=%3d z=%d M=%d m=%d s=%d p=%d P=%d h=%d\n",
							__func__, id, x, y, z, touch_major, touch_minor, size, pressure, palm, hover);
				} else {
					info->m_info[id]++;
					if (info->debug) {
						dev_info(&info->client->dev, "%s: [M][%d] x=%3d, y=%3d z=%d M=%d m=%d s=%d p=%d P=%d h=%d\n",
							__func__, id, x, y, z, touch_major, touch_minor, size, pressure, palm, hover);
					}
				}
			}
				...
				//input_mt_sync(info->input_dev);
#endif
				dev_info(&info->client->dev, "%s: [R][%d] x=%3d, y=%3d, m=%d, v=%s\n",
							__func__, id, info->x_info[id], info->y_info[id], info->m_info[id], info->fw_ver_str);
...
#if MIP_USE_KEY
	struct input_dev *input_tkey = info->input_tkey;
#endif
...
//	input_set_abs_params(input_dev, ABS_MT_TOOL_TYPE, 0, MT_TOOL_MAX, 0, 0);
	...
	set_bit(KEY_BACK, input_tkey->keybit);
	set_bit(KEY_PHONE, input_tkey->keybit);

	info->key_code[0] = KEY_PHONE;
	info->key_disc[0] = "KEY_PHONE";
	info->key_code[1] = KEY_BACK;
	info->key_disc[1] = "KEY_BACK";
#endif
	dev_dbg(&info->client->dev, "%s [DONE]\n", __func__);
	return;
}
...
static int mip_callback_notifier(struct notifier_block *nb, unsigned long noti_type, void *v)
{
