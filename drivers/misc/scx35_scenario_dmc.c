...
static int dmc_fb_notifier_callback(struct notifier_block *self,
                                unsigned long event, void *data)
{
	...
	case FB_BLANK_POWERDOWN:
		printk("%s %d set freq:%d\n", __func__, __LINE__, MIN_DDR_FREQ);
		mutex_lock(&dfs_lock);
		freq = MIN_DDR_FREQ;
		dfs_request(freq);
		mutex_unlock(&dfs_lock);
	...
	case FB_BLANK_UNBLANK:
		printk("%s %d set freq:%d\n", __func__, __LINE__, MAX_DDR_FREQ);
		mutex_lock(&dfs_lock);
		freq = MAX_DDR_FREQ;
		dfs_request(freq);
		mutex_unlock(&dfs_lock);
...
static struct notifier_block dmc_fb_notif =
{
        .notifier_call = dmc_fb_notifier_callback,
};
...
static int __init scenario_dfs_init(void)
{
	...
	fb_register_client(&dmc_fb_notif);
#ifndef CONFIG_PM_DEVFREQ
...
#endif
	fb_unregister_client(&dmc_fb_notif);
#ifndef CONFIG_PM_DEVFREQ	
