...
static int get_pa_info_from_dt(void) {
	int ret = 0;
	const char *vdd_pa;
	...
	ext_pa = of_get_gpio(np, 5);

	if (gpio_is_valid(ext_pa)) {
		ret = gpio_request(ext_pa, "ext_pa");
		if (ret) {
			printk(KERN_ERR"get ext ldo en gpio failed\n");
			return ret;
		}
	} else {
...
void marlin_pa_enable(bool enable){
	...
	if (ext_pa == 0 && download_vdd == NULL) {
		ret = get_pa_info_from_dt();
		if (ret < 0) {
			printk(KERN_ERR"No PA configured in the DTS\n");
		    return;
		}
	}
	...
	if (enable) {
		if (pa_state == 0) {
			if (gpio_is_valid(ext_pa)) {
				gpio_direction_output(ext_pa, 1);
				pa_state++;
			} else {
		...
		if (pa_state == 1) {
			if (gpio_is_valid(ext_pa)) {
				gpio_direction_output(ext_pa, 0);
				pa_state--;
			} else {
