...
int mem_dump_mmu_page_table_wrapper(struct mali_session_data *session_data, _mali_uk_dump_mmu_page_table_s __user *uargs)
{
	...
	/*
	 * A malicious user can craft an application which executes the
	 * aforementioned ioctl call and gets the leaked address, which can be
	 * used to bypass ASLR protection.
	 */
	kargs.ctx = 0;
				...
				_mali_uk_gles_mem_profiler_s __user *uargs)
{
	...
	if (kargs.type >= MALI_MAX_GLES_MEM_TYPES) {
		MALI_PRINT_ERROR(("The type[%u] is unavailable.", kargs.type));
		return -EINVAL;
	}
