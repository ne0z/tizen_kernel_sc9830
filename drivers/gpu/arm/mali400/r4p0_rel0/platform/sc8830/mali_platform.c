...
#ifdef CONFIG_ARCH_SCX30G /*tshark 28nm*/
...
#elif defined(CONFIG_ARCH_SCX35L) /*sharkl 28nm*/
#define DFS_FREQ_NUM			8
...
static struct gpu_clock_source gpu_clk_src[] = {
	{
		.name = "clk_460m8_gpu_gate",
		.freq = 460800,
		.freq_select = 6,
		.clk_src = NULL,
	},
	{
		.name = "clk_384m_gpu_gate",
		.freq = 384000,
		.freq_select = 5,
		.clk_src = NULL,
	},
	{
		.name = "clk_312m_gpu_gate",
		.freq = 312000,
		.freq_select = 4,
		.clk_src = NULL,
	},
	{
		.name = "clk_300m_gpu_gate",
		.freq = 300000,
		.freq_select = 3,
		.clk_src = NULL,
	},
	{
		.name = "clk_256m_gpu_gate",
		.freq = 256000,
		.freq_select = 2,
		.clk_src = NULL,
	},
	{
		.name = "clk_208m_gpu_gate",
		.freq = 208000,
		.freq_select = 1,
		.clk_src = NULL,
	},
	{
		.name = "clk_153m6_gpu_gate",
		.freq = 153600,
		.freq_select = 0,
		.clk_src = NULL,
	},
...
static const int gpu_clk_num =
			sizeof(gpu_clk_src) / sizeof(struct gpu_clock_source);
...
#else /*shark 40nm*/
	.dfs_freq_list = {
		/* index: 0 freq: 312000 freq_select : 3 div_select: 1
			up: 280800 down: 218400 */
		&dfs_freq_full_list[0],
		/* index: 1 freq: 256000 freq_select : 1 div_select: 1
			up: 230400 down: 179200 */
		&dfs_freq_full_list[4],
		/* index: 2 freq: 208000 freq_select : 0 div_select: 1
			up: 187200 down: 145600 */
		&dfs_freq_full_list[8],
		/* index: 3 freq: 156000 freq_select : 3 div_select: 2
			up: 140400 down: 109200 */
		&dfs_freq_full_list[1],
		/* index: 4 freq: 104000 freq_select : 0 div_select: 2
			up:  93600 down:  72800 */
		&dfs_freq_full_list[9],
		/* index: 5 freq:  64000 freq_select : 1 div_select: 4
			up:  57600 down:  44800 */
...
static int get_next_freq(struct gpu_freq_info *freq_list[],
				const struct gpu_freq_info *min_freq,
				const struct gpu_freq_info *max_freq, int key)
{
	int low = max_freq->index, high = min_freq->index, mid = 0;
	...
	while (low <= high) {
		...
		if ((key < freq_list[mid]->up_threshold) &&
			(mid == min_freq->index))
			return mid;
		else if ((key < freq_list[mid]->up_threshold) &&
				(key > freq_list[mid + 1]->up_threshold))
			return mid;
...
/*
	...
	index:12 freq:300000 freq_select:3 div_select:1 up:270000 down:     0
	index:13 freq:150000 freq_select:3 div_select:2 up:135000 down:     0
	index:14 freq:100000 freq_select:3 div_select:3 up: 90000 down:     0
	index:15 freq: 75000 freq_select:3 div_select:4 up: 67500 down:     0
	index:16 freq:256000 freq_select:2 div_select:1 up:230400 down:     0
	index:17 freq:128000 freq_select:2 div_select:2 up:115200 down:     0
	index:18 freq: 85333 freq_select:2 div_select:3 up: 76800 down:     0
	index:19 freq: 64000 freq_select:2 div_select:4 up: 57600 down:     0
	index:20 freq:208000 freq_select:1 div_select:1 up:187200 down:     0
	index:21 freq:104000 freq_select:1 div_select:2 up: 93600 down:     0
	index:22 freq: 69333 freq_select:1 div_select:3 up: 62400 down:     0
	index:23 freq: 52000 freq_select:1 div_select:4 up: 46800 down:     0
	index:24 freq:153600 freq_select:0 div_select:1 up:138240 down:     0
	index:25 freq: 76800 freq_select:0 div_select:2 up: 69120 down:     0
	index:26 freq: 51200 freq_select:0 div_select:3 up: 46080 down:     0
	index:27 freq: 38400 freq_select:0 div_select:4 up: 34560 down:     0
*/

	for (i = 0; i < gpu_clk_num; i++) {
		for (j = 0; j < GPU_HARDWARE_MAX_DIVISION; j++) {
			dfs_freq_full_list[i * 4 + j].index = i * 4 + j;
			dfs_freq_full_list[i * 4 + j].freq =
						gpu_clk_src[i].freq / (j + 1);
			dfs_freq_full_list[i * 4 + j].freq_select =
						gpu_clk_src[i].freq_select;
			dfs_freq_full_list[i * 4 + j].div_select = j + 1;
			dfs_freq_full_list[i * 4 + j].up_threshold =
				gpu_clk_src[i].freq * UP_THRESHOLD / (j + 1);
			dfs_freq_full_list[i * 4 + j].clk_src =
	...
	for (i = 0; i < gpu_clk_num * GPU_HARDWARE_MAX_DIVISION; i++) {
		MALI_DEBUG_PRINT(3,
			("full list index:%3d freq:%6d freq_select:%3d  div_select:%3d up:%6d  down:%6d\n",
					dfs_freq_full_list[i].index,
					dfs_freq_full_list[i].freq,
					dfs_freq_full_list[i].freq_select,
					dfs_freq_full_list[i].div_select,
					dfs_freq_full_list[i].up_threshold,
					dfs_freq_full_list[i].down_threshold));
	}
}
...
static void gpu_dfs_context_init(void)
{
	...
	for (i = 0; i < gpu_dfs_ctx.freq_list_len; i++) {
		...
		MALI_DEBUG_PRINT(2,
			("index:%3d freq:%6d freq_select:%3d  div_select:%3d up:%6d  down:%6d\n",
				gpu_dfs_ctx.dfs_freq_list[i]->index,
				gpu_dfs_ctx.dfs_freq_list[i]->freq,
				gpu_dfs_ctx.dfs_freq_list[i]->freq_select,
				gpu_dfs_ctx.dfs_freq_list[i]->div_select,
				gpu_dfs_ctx.dfs_freq_list[i]->up_threshold,
				gpu_dfs_ctx.dfs_freq_list[i]->down_threshold));
	}

	i = freq_search(gpu_dfs_ctx.dfs_freq_list, gpu_dfs_ctx.freq_list_len,
			DFS_MAX_FREQ);
	if (i < 0) {
		MALI_PRINT_ERROR(("invalid DFS_MAX_FREQ:%d\n", DFS_MAX_FREQ));
	...
	i = freq_search(gpu_dfs_ctx.dfs_freq_list, gpu_dfs_ctx.freq_list_len,
			DFS_MIN_FREQ);
	if (i < 0) {
		MALI_PRINT_ERROR(("invalid DFS_MIN_FREQ:%d\n", DFS_MIN_FREQ));
		gpu_dfs_ctx.dfs_min_freq_p =
				gpu_dfs_ctx.dfs_freq_list[DFS_FREQ_NUM - 1];
	...
	i = freq_search(gpu_dfs_ctx.dfs_freq_list, gpu_dfs_ctx.freq_list_len,
			GPU_MAX_FREQ);
	if (i < 0) {
		MALI_PRINT_ERROR(("invalid GPU_MAX_FREQ:%d\n", GPU_MIN_FREQ));
	...
	i = freq_search(gpu_dfs_ctx.dfs_freq_list, gpu_dfs_ctx.freq_list_len,
			GPU_MIN_FREQ);
	if (i < 0) {
		MALI_PRINT_ERROR(("invalid GPU_MIN_FREQ:%d\n", GPU_MIN_FREQ));
...
static struct resource mali_gpu_resources[] = {
	...
	MALI_GPU_RESOURCES_MALI400_MP4_PMU(SPRD_MALI_PHYS,
						IRQ_GPU_INT, IRQ_GPU_INT,
						IRQ_GPU_INT, IRQ_GPU_INT,
						IRQ_GPU_INT, IRQ_GPU_INT,
						IRQ_GPU_INT, IRQ_GPU_INT,
						IRQ_GPU_INT, IRQ_GPU_INT)
#elif MALI_PP_CORE_NUMBER == 2
	MALI_GPU_RESOURCES_MALI400_MP2_PMU(SPRD_MALI_PHYS,
						IRQ_GPU_INT, IRQ_GPU_INT,
						IRQ_GPU_INT, IRQ_GPU_INT,
						IRQ_GPU_INT, IRQ_GPU_INT)
#else
	MALI_GPU_RESOURCES_MALI400_MP1_PMU(SPRD_MALI_PHYS,
						IRQ_GPU_INT, IRQ_GPU_INT,
						IRQ_GPU_INT, IRQ_GPU_INT)
#endif
};
...
int mali_power_initialize(struct platform_device *pdev)
{
...
#ifdef CONFIG_ARCH_SCX30G /*tshark 28nm*/
	gpu_clk_src[0].clk_src = of_clk_get(np, 8);
	gpu_clk_src[1].clk_src = of_clk_get(np, 7);
	gpu_clk_src[2].clk_src = of_clk_get(np, 6);
	gpu_clk_src[3].clk_src = of_clk_get(np, 5);
	gpu_clk_src[4].clk_src = of_clk_get(np, 4);
	gpu_clk_src[5].clk_src = of_clk_get(np, 3);
	gpu_clk_src[6].clk_src = of_clk_get(np, 2);
#elif defined(CONFIG_ARCH_SCX35L) /*sharkl 28nm*/
...
#ifdef CONFIG_COMMON_CLK
	clk_prepare_enable(gpu_clk_src[3].clk_src);
#else
	clk_enable(gpu_clk_src[3].clk_src);
#endif
#endif
	...
	if (!gpu_dfs_ctx.gpu_power_on) {
		...
		sci_glb_clr(REG_PMU_APB_PD_GPU_TOP_CFG,
		...
		clk_set_parent(gpu_dfs_ctx.gpu_clock,
					gpu_dfs_ctx.dfs_max_freq_p->clk_src);
	...
	if (!gpu_dfs_ctx.gpu_dfs_workqueue)
		gpu_dfs_ctx.gpu_dfs_workqueue =
				create_singlethread_workqueue("gpu_dfs");
...
int mali_platform_device_register(void)
{
	...
	if (gpu_dfs_ctx.gpu_power_on) {
		...
		sci_glb_set(REG_PMU_APB_PD_GPU_TOP_CFG,
						BIT_PD_GPU_TOP_FORCE_SHUTDOWN);
	}
...
void mali_platform_power_mode_change(int power_mode)
{
	...
	case 1: /*MALI_POWER_MODE_LIGHT_SLEEP*/
		...
		if (sci_glb_read(REG_PMU_APB_PD_GPU_TOP_CFG,
						BIT_PD_GPU_TOP_FORCE_SHUTDOWN)) {
			gpu_cur_freq = 0;
#ifdef CONFIG_SYSTEM_LOAD_ANALYZER
...
static inline void mali_set_div(int clock_div)
{
	...
	sci_glb_write(REG_GPU_APB_APB_CLK_CTRL,
			BITS_CLK_GPU_DIV(clock_div - 1), BITS_CLK_GPU_DIV(3));
}
...
static inline void mali_set_freq(u32 gpu_freq)
{
	...
	sci_glb_write(REG_GPU_APB_APB_CLK_CTRL,
			BITS_CLK_GPU_SEL(gpu_freq), BITS_CLK_GPU_SEL(3));
}
...
{
	int max_freq_index, min_freq_index, target_freq, next_freq_index;
	...
	switch (gpu_level) {
	case 10:
		max_freq_index = freq_search(gpu_dfs_ctx.dfs_freq_list,
						gpu_dfs_ctx.freq_list_len,
						gpufreq_max_limit);
		if (max_freq_index < 0) {
			...
			gpu_dfs_ctx.dfs_max_freq_p =
				gpu_dfs_ctx.dfs_freq_list[max_freq_index];
			gpu_dfs_ctx.dfs_min_freq_p =
				gpu_dfs_ctx.dfs_freq_list[max_freq_index];
		}
		gpu_level = 1;
	...
	case 9:
		...
		if (max_freq_index < 0) {
			gpu_dfs_ctx.dfs_max_freq_p =
						gpu_dfs_ctx.default_max_freq_p;
			gpu_dfs_ctx.dfs_min_freq_p =
						gpu_dfs_ctx.default_max_freq_p;
		} else {
			gpu_dfs_ctx.dfs_max_freq_p =
	...
	case 7:
		...
		if (max_freq_index < 0) {
			gpu_dfs_ctx.dfs_max_freq_p =
				gpu_dfs_ctx.dfs_freq_list[GPU_150M_FREQ_INDEX];
			gpu_dfs_ctx.dfs_min_freq_p =
				gpu_dfs_ctx.dfs_freq_list[GPU_150M_FREQ_INDEX];
		} else {
			gpu_dfs_ctx.dfs_max_freq_p =
	...
	default:
		max_freq_index = freq_search(gpu_dfs_ctx.dfs_freq_list,
						gpu_dfs_ctx.freq_list_len,
						gpufreq_max_limit);
		min_freq_index = freq_search(gpu_dfs_ctx.dfs_freq_list,
						gpu_dfs_ctx.freq_list_len,
						gpufreq_min_limit);
		if (max_freq_index < 0)
			gpu_dfs_ctx.dfs_max_freq_p =
		...
		if (min_freq_index < 0)
			...
			gpu_dfs_ctx.dfs_min_freq_p =
				gpu_dfs_ctx.dfs_freq_list[min_freq_index];
		break;
	}
	...
	if (gpu_dfs_ctx.cur_load >= (256 * UP_THRESHOLD))
		...
		target_freq = gpu_dfs_ctx.cur_freq_p->freq *
						gpu_dfs_ctx.cur_load / 256;
	...
	MALI_DEBUG_PRINT(3,
	("GPU_DFS gpu util %3d: target_freq:%6d cur_freq %6d-> next_freq %6d\n",
					gpu_dfs_ctx.cur_load, target_freq,
					gpu_dfs_ctx.cur_freq_p->freq,
					gpu_dfs_ctx.next_freq_p->freq));
	...
	if (gpu_dfs_ctx.next_freq_p->freq != gpu_dfs_ctx.cur_freq_p->freq) {
		...
		if (gpu_dfs_ctx.gpu_dfs_workqueue)
			queue_work(gpu_dfs_ctx.gpu_dfs_workqueue,
								&gpu_dfs_work);
#else
		if (gpu_dfs_ctx.next_freq_p->freq_select !=
					gpu_dfs_ctx.cur_freq_p->freq_select) {
			if (gpu_dfs_ctx.gpu_dfs_workqueue)
				queue_work(gpu_dfs_ctx.gpu_dfs_workqueue,
								&gpu_dfs_work);
		} else if (gpu_dfs_ctx.next_freq_p->div_select !=
					gpu_dfs_ctx.cur_freq_p->div_select) {
			down(gpu_dfs_ctx.sem);
			if (gpu_dfs_ctx.gpu_power_on &&
						gpu_dfs_ctx.gpu_clock_on) {
				mali_set_div(
					gpu_dfs_ctx.next_freq_p->div_select);
				gpu_dfs_ctx.cur_freq_p =
						gpu_dfs_ctx.next_freq_p;
				gpu_cur_freq = gpu_dfs_ctx.cur_freq_p->freq;
			}
			up(gpu_dfs_ctx.sem);
		}
...
static void gpufreq_limit_init(void)
{
	gpufreq_table = kzalloc(256 * sizeof(char), GFP_KERNEL);
...
static void gpu_change_freq_div(void)
{
	...
	MALI_DEBUG_PRINT(3,
	("GPU_DFS set clk cur freq %6d-> next freq %6d next clk_src 0x%p\n",
