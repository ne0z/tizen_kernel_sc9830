...
/*
  * data, skb->data
  * direction, tx=1 or rx=0
  */
void wlan_dump_data(unsigned char *data, bool dir);

/*
 * data, skb->data
 * direction, tx=1 or rx=0
 */
void wlan_dump_eapol(unsigned char *data, bool dir);
...
void wlan_dump_data(unsigned char *data, bool dir)
{
	...
	if (eh->ether_type == htons(ETHER_TYPE_802_1X)) {
		wlan_dump_eapol(data, dir);
	}
}
...
void wlan_dump_eapol(unsigned char *data, bool dir)
{
	...
	int pair, ack, mic, err, req, sec, install;
	unsigned short info;
	desc_type = data[18];
	if (desc_type == 2 || desc_type == 254) {
		info = (data[19] << 8) | data[20];
		pair = 0 != (info & 0x08);
		install  = 0 != (info & 0x40);
		ack = 0  != (info & 0x80);
		mic = 0  != (info & 0x100);
		...
		err = 0  != (info & 0x400);
		req = 0  != (info & 0x800);
		if (!sec && !mic && ack && !install && pair && !err && !req) {
			printke("ETHER_TYPE_802_1X [%s] : msg 1 of 4-Way\n",
				dir ? "TX" : "RX");
		} else if (pair && !install && !ack && mic && !sec && !err && !req) {
			printke("ETHER_TYPE_802_1X [%s] : msg 2 of 4-Way\n",
				dir ? "TX" : "RX");
		} else if (pair && ack && mic && sec && !err && !req) {
			printke("ETHER_TYPE_802_1X [%s] : msg 3 of 4-Way\n",
				dir ? "TX" : "RX");
		} else if (pair && !install && !ack && mic && sec && !req && !err) {
			printke("ETHER_TYPE_802_1X [%s] : msg 4 of 4-Way\n",
				dir ? "TX" : "RX");
		} else {
			printke("ETHER_TYPE_802_1X [%s]: ver %d, type %d, replay %d\n",
				dir ? "TX" : "RX",
				data[14], data[15], data[30]);
		}
	} else {
		printke("ETHER_TYPE_802_1X [%s]: ver %d, type %d, replay %d\n",
				dir ? "TX" : "RX",
	...
	hex_dump("", strlen(""), data, 18 + ((data[16] << 8) | data[17]));
}
...
#if defined(WLAN_DUMP_DATA)
	wlan_dump_data(skb->data, WLAN_RX);
#endif
...
#if defined(WLAN_DUMP_DATA)
	wlan_dump_data(skb->data, WLAN_TX);
#endif
...
int marlin_priv_cmd(struct net_device *ndev, struct ifreq *ifr)
{
	...
	} else if (strncmp(command, "COUNTRY", strlen("COUNTRY")) == 0) {
		wlan_set_country_code(ndev, ifr);
	} else {
...
static int wlan_ioctl(struct net_device *ndev, struct ifreq *req, int cmd)
{
	...
	/* We should eliminate it, becase it's already in "SIOCDEVPRIVATE + 1". */
				...
				unsigned long event, void *data)
{
	...
	if((fb_event < 0xc0000000) || (fb_event->data < 0xc0000000) )
		return 0;
	power = *(int *)fb_event->data;
...
static struct notifier_block wlan_fb_notif =
{
...
#ifdef CONFIG_SLEEP_MONITOR
int wlan_get_sleep_monitor_cb(void *priv, unsigned int *raw_val,
		int check_level, int caller_type)
{
	int state = DEVICE_ON_ACTIVE1, type = check_level;
	wlan_vif_t *vif_wlan = &(g_wlan.netif[NETIF_0_ID]);
	wlan_vif_t *vif_p2p = &(g_wlan.netif[NETIF_1_ID]);
	unsigned int temp_raw = 0;

	if (type = SLEEP_MONITOR_CHECK_HARD) {
	}

	if (vif_p2p->mode == ITM_P2P_CLIENT_MODE)
		temp_raw |= 0x2 << 12;
	else if (vif_p2p->mode == ITM_P2P_GO_MODE)
		temp_raw |= 0x3 << 12;
	else if (vif_wlan->mode == ITM_STATION_MODE)
		temp_raw |= 0x1 << 12;
	else if (vif_wlan->mode == ITM_AP_MODE)
		temp_raw |= 0x4 << 12;
	else
		temp_raw |= 0x5 << 12;

	if (strcmp(vif_wlan->cfg80211.ssid, "") != 0 ||
		strcmp(vif_p2p->cfg80211.ssid, "") != 0)
		temp_raw |= 1 << 8;

	if (vif_wlan->cfg80211.scan_request != NULL ||
		vif_p2p->cfg80211.scan_request != NULL) {
		temp_raw |= 1 << 4;
...
static struct sleep_monitor_ops wlan_sleep_monitor_ops = {
	.read_cb_func = wlan_get_sleep_monitor_cb,
};
#endif
...
#else
	fb_register_client(&wlan_fb_notif);
#endif
...
#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_register_ops(NULL, &wlan_sleep_monitor_ops,
			SLEEP_MONITOR_WIFI);
#endif
	printke("%s ok!\n", __func__);
	return OK;
...
#else
	fb_unregister_client(&wlan_fb_notif);
#endif
