...
#ifdef CONFIG_FB_ESD_DETECT_BY_INT
static struct esd_det_info esd_context;
#endif
...
static int32_t sprd_panel_enter_sleep(struct panel_spec *self, uint8_t is_sleep)
{
	...
	if (is_sleep)
		...
		gen_panel_set_status(lcd, GEN_PANEL_SLEEP_OUT);
...
#if defined(CONFIG_FB_ESD_DETECT_BY_INT) && defined(CONFIG_FB_LCD_DEBUG)
...
#if defined(CONFIG_FB_ESD_DETECT_BY_INT) && defined(CONFIG_FB_LCD_DEBUG)
static DEVICE_ATTR(esd_check, 0644, sprdfb_esd_check_show,
		sprdfb_esd_check_store);
#endif
...
#ifdef CONFIG_FB_ESD_DETECT_BY_INT
	sprdfb_panel_esd_detect_init();
#ifdef CONFIG_FB_LCD_DEBUG
	ret = device_create_file(lcd->dev, &dev_attr_esd_check);
	if (unlikely(ret < 0)) {
...
static void sprdfb_panel_esd_uevent_notifier(struct work_struct *work)
{
	...
	if (!lcd->active) {
		pr_warn("%s: Display off Skip ESD recovery\n", __func__);
		return;
	}
...
static int sprdfb_panel_esd_detect_init()
{
	...
	if (result) {
		pr_err("%s: esd_det gpio request failed, Error[%d]\n", __func__,
				result);
		goto err_exit;
	}
	...
	if (unlikely(result < 0)) {
		pr_err("%s: failed to set esd_det gpio %d as input, Error[%d]\n",
				__func__, esd_context.esd_det_gpio, result);
	...
	if (result) {
		pr_err("%s: esd_detect irq request failed, Error[%d]", __func__,
				result);
	...
	if (result) {
		pr_err("%s: err_fg gpio request failed, Error[%d]\n", __func__, result);
		goto err_exit;
	}
	...
	if (unlikely(result < 0)) {
		pr_err("%s: failed to set err_fg gpio %d as input, Error[%d]\n",
				__func__, esd_context.err_fg_gpio, result);
	...
	result = request_threaded_irq(esd_context.err_fg_irq, NULL,
			...
			"err_fg", NULL);
	if (result) {
		pr_err("%s: err_fg irq request failed, Error[%d]", __func__, result);
