...
void cp0_power_domain_debug(void)
{
        ...
        pr_debug("###---- REG_PMU_APB_PD_CP0_ARM9_0_CFG : 0x%08x\n", cp0_arm9_0_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP0_ARM9_1_CFG : 0x%08x\n", cp0_arm9_1_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP0_HU3GE_CFG : 0x%08x\n", cp0_hu3ge_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP0_GSM_0_CFG : 0x%08x\n", cp0_gsm_0_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP0_GSM_1_CFG : 0x%08x\n", cp0_gsm_1_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP0_TD_CFG : 0x%08x\n", cp0_td_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP0_CEVA_0_CFG : 0x%08x\n", cp0_ceva_0_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP0_CEVA_1_CFG : 0x%08x\n", cp0_ceva_1_cfg);
}
...
void cp1_power_domain_debug(void)
{
        ...
        pr_debug("###---- REG_PMU_APB_PD_CP1_CA5_CFG : 0x%08x\n", cp1_ca5_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP1_LTE_P1_CFG : 0x%08x\n", cp1_lte_p1_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP1_LTE_P2_CFG : 0x%08x\n", cp1_lte_p2_cfg);
        pr_debug("###---- REG_PMU_APB_PD_CP1_CEVA_CFG : 0x%08x\n", cp1_ceva_cfg);
...
static void print_debug_info(void)
{
	...
	pr_debug("###---- REG_PUB_APB_DDR_ID2QOS_RCFG9 : 0x%08x\n", ca5_lte_status);
...
#endif
#if defined(CONFIG_ARCH_SCX35L)
	pr_debug("###---- REG_AON_APB_MPLL_CFG2 : 0x%08x\n", mpll_cfg2);
        pr_debug("###---- REG_AON_APB_DPLL_CFG2 : 0x%08x\n", dpll_cfg2);
#endif
#if defined(CONFIG_ARCH_SCX15)
	...
	if (ahb_eb & BIT_HSIC_EB)
                pr_debug("###---- BIT_HSIC_EB still set ----###\n");
	if (ahb_eb & BIT_DISPC_EB)
                pr_debug("###---- BIT_DISPC_EB still set ----###\n");
#else
...
void compareGoldenLight(void)
{
	...
	pr_debug("REG_AP_AHB_MCU_PAUSE=0x%x \n",gREG_AP_AHB_MCU_PAUSE);
	pr_debug("REG_AP_CLK_GSP_CFG(gsp_busy)= 0x%x \n",gsp_busy);
	pr_debug("DMA_REG_DMA_DEBUG_STATUS=0x%x \n",__raw_readl((void *)DMA_REG_DMA_DEBUG_STATUS));
	pr_debug("REG_AP_AHB_CA7_STANDBY_STATUS 0x=%x \n",__raw_readl((void *)REG_AP_AHB_CA7_STANDBY_STATUS));
	pr_debug("REG_AON_APB_APB_EB0 0x=%x \n",__raw_readl((void *)REG_AON_APB_APB_EB0));
	pr_debug("REG_AP_AHB_AHB_EB 0x=%x \n",__raw_readl((void *)REG_AP_AHB_AHB_EB));
	pr_debug("REG_AP_AHB_AP_SYS_AUTO_SLEEP_CFG 0x=%x \n",__raw_readl((void *)REG_AP_AHB_AP_SYS_AUTO_SLEEP_CFG));
...
void showAPLightSleepStatus(void)
{
	...
	pr_debug(KERN_INFO "Note: light sleep cannot read dispc0_busy,dispc1_busy status\n");

	if( ap_light_sleep_req == 0 ){
		if ( zipdec_ckg_en_eb == 0)
			pr_debug("*. zipdec_ckg_en_eb = 0 \n");
		if ( zipenc_ckg_en_eb == 0)
			pr_debug("*. zipenc_ckg_en_eb = 0 \n");
		if ( dispc1_eb == 0)
			pr_debug("*. dispc1_eb = 0, but need to double check dispc1_busy \n");
		if ( dispc0_eb == 0)
			pr_debug("*. dispc0_eb = 0, but need to double check dispc0_busy \n");
		if( gsp_ckg_auto_en_eb == 1 ){
			pr_debug("*. gsp_ckg_auto_en_eb = 1 \n");
			pr_debug("*. gsp_auto_gate_en = %d gsp_busy=%d (1,1) will not enter light\n",gsp_auto_gate_en,gsp_busy);
			pr_debug("*. gsp_ckg_force_en = %d \n",gsp_ckg_force_en);
		}
		if(mcu_light_stop == 0 )
			pr_debug("*. mcu_light_stop = 0 mmtx_light_stop=%d mcu_light_sleep_en=%d\n",mmtx_light_stop,mcu_light_sleep_en);


		if((ca7standby == 0) && (__raw_readl((void *)REG_AP_AHB_CA7_STANDBY_STATUS)!= 0xfee))
			pr_debug("ca7standby = 0x%x, if(val == 0xfee)some core not in wfi mode\n",__raw_readl((void *)REG_AP_AHB_CA7_STANDBY_STATUS));
		//0xfee: standbywfi2 and standbywfi[0] not 1, it correct at this point


		if( mcu_light_stop == 0 ){
			pr_debug("** mcu_light_stop = 0 \n");
			if( mcu_light_sleep_en == 0 )
				pr_debug("**. mcu_light_sleep_en = 0 \n");
			if( mmtx_light_stop == 0 ){
				pr_debug("**. mmtx_light_stop = 0 \n");
				if ( mcu_sleep_follow_ca7 == 0)
					pr_debug("@ mcu_sleep_follow_ca7 = 0 mean each driver must turn off by itself @\n");
				if( GET_BIT_EB(REG_AON_APB_APB_EB0,30) == 0 )
					pr_debug("***. ca7_dap_eb = 0 \n");
				...
				if( GET_BIT_EB(REG_AP_AHB_AHB_EB,17) == 0 )
					pr_debug("***. nandc_eb = 0 \n");
				if(ca7_sleep == 0){
					pr_debug("** ca7_sleep==0\n");

					if(ca7standby == 0)
						pr_debug("*** ca7standby=0 ca7standby_status=0x%x (?=0xfee)\n",__raw_readl((void *)REG_AP_AHB_CA7_STANDBY_STATUS));
					if((ca7standby == 0) && (__raw_readl((void *)REG_AP_AHB_CA7_STANDBY_STATUS)!= 0xfee))
						pr_debug("ca7standby = 0x%x, if(val == 0xfee)some core not in wfi mode\n",__raw_readl((void *)REG_AP_AHB_CA7_STANDBY_STATUS));
					//0xfee: standbywfi2 and standbywfi[0] not 1, it correct at this point
				}
				...
				if( dma_act_light_en == 0 )
					pr_debug("@* dma_act_light_en = 0 it will not effect light sleep\n *@");
				if( mcu_sleep_follow_ca7_en == 0)
					pr_debug("@ mcu_sleep_follow_ca7_en = 0 for_test not effect result@\n");
			}
			pr_debug("? ap_wakeup_nirq cannot be read");
		}

	}
	if( mcu_core_sleep == 1)
		pr_debug("ERROR!!force A7 into sleep by software. mcu_core_sleep = 1 it's for debug, it should not be 1\n");
...
void showAPDeepSleepStatus(void)
{
	...
	pr_debug("deepsleep: cannot check ap_wakeup_nirq val\n");
	if((peri_stop == 1) && (mcu_deep_stop == 1)){
		ap_deep_sleep_req = 1;
		pr_debug("in deep sleep mode\n");
	}else{
		if(peri_stop == 0){
