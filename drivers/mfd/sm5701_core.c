...
void SM5701_clear_operationmode(int operation_mode)
{
	struct i2c_client * client;
	u8 data = 0;
...
EXPORT_SYMBOL_GPL(SM5701_clear_operationmode);
...
int SM5701_operation_mode_function_control(void)
{
        ...
        if ((pdata->charger_data->cable_type == POWER_SUPPLY_TYPE_MAINS) ||
                        ...
                        (pdata->charger_data->cable_type == POWER_SUPPLY_TYPE_UPS))
        {
                if (led_ready_state == LED_FLASH)
                {
                        SM5701_set_bstout(SM5701_BSTOUT_4P7);
                        SM5701_set_operationmode(SM5701_OPERATIONMODE_FLASH_ON);
                }
