...
static struct kretprobe set_backlight_krp = {
	...
	.data_size = sizeof(struct maru_data)
};
...
static unsigned int maru_power_state = 0;

static int entry_handler_set_backlight(struct kretprobe_instance *ri,
				       struct pt_regs *regs)
{
	struct maru_data *data = (struct maru_data *)ri->data;
	struct backlight_device *bd = (struct backlight_device *)swap_get_karg(regs, 0);

	data->brightness = bd->props.brightness;
	data->state = bd->props.state;
				     ...
				     struct pt_regs *regs)
{
	...
	if (!ret && maru_ops.notifier) {
		/* we check only state #2, TODO checking other state */
		if (maru_power_state != data->state) {
			int power;
			if (data->state == 2)
			...
			maru_ops.notifier(&maru_ops, LAT_POWER,
					  (void *)power);
			maru_power_state = data->state;
		} else {
			maru_ops.notifier(&maru_ops, LAT_BRIGHTNESS,
					  (void *)data->brightness);
		}
	}
