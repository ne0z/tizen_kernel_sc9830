...
#define CP_DEBUG_BUF_SIZE	256
static ssize_t restart_cpc_store(struct kobject *kobj, struct kobj_attribute *attr,
					const char *buf, size_t n)
{
	int val = 0;
	char cp_debug_info[CP_DEBUG_BUF_SIZE] = {0,};

	if (sscanf(buf, "%d", &val) == 1 && val > 0) {
		switch (val) {
		case 1:
			snprintf(cp_debug_info, CP_DEBUG_BUF_SIZE - 1, "CP Assert %s", buf+2);
			break;
		case 2:
			snprintf(cp_debug_info, CP_DEBUG_BUF_SIZE - 1, "Telephony Force Assert %s", buf+2);
			break;
		default:
			snprintf(cp_debug_info, CP_DEBUG_BUF_SIZE - 1, "CP Crash %s", buf+2);

		}

		cp_abort(cp_debug_info);
	}
	return n;
}
power_attr(restart_cpc);
